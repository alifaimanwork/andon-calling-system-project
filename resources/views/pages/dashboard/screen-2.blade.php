...
@section('body')
    <main>
        <div class="line-container flex-grow-1">
            ...
            <div class="p-3 production-line" data-line-no="2">
        ...
        <!-- <div class="temp-display">Runtime: <span class="live-runtime-timer" data-tag="plan" data-format="timer_full"></span></div> -->
    </main>
@endsection
@section('templates')
    @parent
...
@section('scripts')
    ...
    <script>
        // Start Live Production Script //
        // var liveProduction = new LiveProduction("{{ date('c') }}");
        // var liveProductionDataSummary = null;

        // function initializeLiveProduction() {
        //     syncLiveProduction();

        //     liveProduction.listenAnyChanges(function(e) {
        //         liveProductionDataSummary = e;
        //         updateLiveData();
        //     });

        //     liveProductionDataSummary = liveProduction.getSummary();
        ...
        //     liveProduction.setTerminalData(terminalData);
        // }
        // End Live Production Script //
    </script>
    ...
    <script>
        // Function to update the footer status text and color
        const updateStatusBar = function() {
            let { status, status_text, downtime } = workCenterStatus;
            const statusBar = document.getElementById('footer-status');
            statusBar.innerHTML = `${downtime || ''} ${status_text || ''}`;
        ...
        // Function to add variance indicator
        const addVarianceIndicator = function(data, varianceLimit) {
            let limit = parseFloat(varianceLimit);
            let variance = parseFloat(data);
            let indicator = '';

            if (!isNaN(limit) && !isNaN(variance)) {
                if (variance > limit) indicator = 'positive';
                if (variance < -1 * limit) indicator = 'negative';
            }
            return indicator;
        }
        ...
        let currentScreen = 1;
        const flipScreen = function() {
            const statusBar = document.getElementById('footer-status');
            if (statusBar.classList.contains('status-human-downtime')) {
                ...
                currentScreen = currentScreen === 1 ? 7 : 1;

                // Construct new URL
                const newUrl = `${url.origin}/dashboard/${plantUid}/${workCenterUid}/screen/${currentScreen}`;
                window.location.href = newUrl;
            }
        }

        // Check status and flip screen if needed
        const checkAndFlipScreen = function() {
            const statusBar = document.getElementById('footer-status');
            if (statusBar.classList.contains('status-human-downtime')) {
                flipScreen();
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            setInterval(checkAndFlipScreen, 3000); // Check every 10 seconds
        });
    </script>
@endsection
